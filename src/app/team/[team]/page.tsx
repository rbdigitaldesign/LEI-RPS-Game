...
export default function TeamPage() {
  ...
  const rawTeamName = params?.team as string;
    ...
    if (tournament && teamName) {
      ...
      if (pod) {
        ...
        if (match) {
          ...
          const teamMove = teamIsPod1 ? (match.moves as any)?.pod1 : (match.moves as any)?.pod2;
          setHasSubmittedMove(!!teamMove);

          if (match.moveHistory && match.moveHistory.length > lastMoveHistoryLength) {
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <main className="flex-grow container mx-auto p-4 flex items-center justify-center">
          <Card className="w-full max-w-md text-center">
            <CardHeader>
              <CardTitle className="text-destructive">Invalid Team</CardTitle>
            </CardHeader>
            <CardContent>
