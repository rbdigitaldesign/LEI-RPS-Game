...
const MatchCard = ({ match, isCurrent }: { match: Match; isCurrent: boolean }) => {
  ...
  if (match.isBye) {
    return (
      <div className="flex items-center w-full">
        <div className="relative w-full bg-card/50 border border-dashed border-primary/50 p-1 text-[10px] h-8 flex items-center">
  ...
  return (
    <div className="relative flex flex-col justify-center w-full">
      <div className={cn(
        "relative w-full bg-card border border-primary/50 p-1 my-0.5 text-[10px] h-8 flex flex-col justify-around transition-all",
  ...
  return (
    <Card className="bg-card border-2 sticky top-20">
      ...
      <CardContent className="p-2">
        <div className="grid gap-x-2" style={{ gridTemplateColumns: `repeat(${rounds.length}, 1fr)`}}>
