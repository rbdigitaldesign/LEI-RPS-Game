...
import { Loader, Swords, Shield, Hourglass, Check, Trophy } from 'lucide-react';

const PodDisplay = ({ pod, opponent, move, submitted }: { pod: Pod | null, opponent: Pod | null, move?: Move, submitted?: boolean }) => {
  if (!pod) return <div />;
  return (
    <div className="flex flex-col items-center text-center">
      ...
      {submitted && (
        <motion.div
            ...
            className="flex items-center gap-2 mt-4 bg-green-500/20 text-green-300 px-3 py-1 rounded-full text-sm"
        >
...
export function TeamPageContent({ teamName }: { teamName: string }) {
  const { tournament, isProcessing, refetch } = useServerTournament();
  ...
  const teamPod = tournament?.pods.find(p => p.name === teamName) || null;
  ...
  const handleSubmitMove = async () => {
    ...
    try {
      ...
      refetch(); // Refetch state to confirm move
    } catch (err: any) {
            ...
            <CardContent>
                ...
                <div className="flex justify-center gap-4">
                    ...
                    <motion.div key={move} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                        ...
                        className={cn(
                            "w-32 h-32 flex flex-col gap-2 transition-all duration-200 border-4",
                            selectedMove === move ? 'border-accent' : 'border-secondary',
                        )}
                        >
                ...
                <Button
                    ...
                    size="lg"
                    className="mt-8 w-full max-w-xs font-bold text-lg"
                >
